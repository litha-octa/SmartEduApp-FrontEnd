...
import { allClass } from "./redux/ActionCreators/allclass";
            ...
            {allClassReducer.isPending ? (
              ...
              <ol>
                {allClassReducer.isFulfilled
                  ? allClassReducer.results.map((char) => (
...
const mapStateToProps = (state) => {
  const {mathReducer,allClassReducer } = state;
  return {
    mathReducer,
    allClassReducer,
  };
...
const mapDispatchToProps = (dispatch) => {
  return {
    getAllClass: () =>
      dispatch(
        allClass("http://localhost:8300/api/v1/newclass/")
      ),
  };
};
